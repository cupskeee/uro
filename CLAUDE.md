...
Each increment got a 5-dimension adversarial-review workflow (find→verify, default-rejected): 2.1 → 4 fixes (branch-name `UNIQUE`, `create_marker FOR UPDATE`, `PlaceStateChanged` enum bar); 2.2 → 1 fix (acceptance test now asserts an active PC binding survives a fork — guarding the snapshot `pcs` path). Next: **Phase 3 — mechanics** (ruleset port + Uro Basic, encounter mode, affordance planner; seeded-RNG replay).
