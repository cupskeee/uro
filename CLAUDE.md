...
**Phase 2 inc 2.1 (timeline substrate) is code-complete, 86 tests green** (fully deterministic, no key needed): commit-depth, `markers`, `snapshots` + materialization-at-any-commit (nearest-snapshot + replay), copy-on-fork branching (`fork_branch` — projections rebuilt at the fork commit, memory-index rows copied, embeddings shared by content-hash), the `proj_places` projection (the meteor mechanic: fork-before-strike keeps Vel standing, fork-after shows the crater, siblings never contaminate), and CLI `uro branch fork/list/mark` + `uro log`. Adversarially reviewed (11 candidate findings → 4 fixed: branch-name uniqueness `UNIQUE(world_id,name)`, `create_marker` `FOR UPDATE`, `PlaceStateChanged` enum bar; the rest were verifier-rejected coverage gaps on correct code). Next: **inc 2.2 — PC binding (`PCBound`/`PCReleased`), campaign-over-branch plumbing, deterministic time-skip (`TimeAdvanced`/`AdaptationApplied`), and the full meteor acceptance test.**
