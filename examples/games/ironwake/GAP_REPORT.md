...
| A trigger quantified over a faction's members ("on ANY Red Band death") — and a parse-time check that a `where` filter CAN ever match | The grammar does NOT refuse it: `Trigger.where` is a free-form `dict[str,str]` matched verbatim against payload fields (`engines/rules.py:65`), so `{"actor.member_of": "f:red-band"}` VALIDATES cleanly and then silently never fires — no `ActorDied` payload carries that key, and nothing warns at parse, import, or runtime. Accepted-but-inert is a sharper authoring footgun than the loud refusals above | the war ratchet triggers on ANY `ActorDied` instead (over-broad but live) | major | Validate where-keys against the trigger event's payload schema at `RulePack` parse (the event catalog is typed — the check is mechanical), plus a real join/quantifier primitive | `cli/season.py counter_wall` (live probe: "ACCEPTED by the grammar — yet it can never fire"); `world/rules.py` RL-6 |
