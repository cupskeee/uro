"""The running friction + refusal log — IRONWAKE's scientific instrument.

Every entry is written AT THE CALL SITE, the moment the Uro API surprised, refused, downgraded,
or forced state into game code (the brief: "keep a running friction log from line one").
GAP_REPORT.md is assembled from THIS, not reconstructed from memory; the season prints the live
...
- REFUSALS -> the Reaction-Layer refusal log (stress goal 8): every rule IRONWAKE wanted but the
...
class RefusalEntry:
    ...
    missing: str  # the missing primitive (counter / arithmetic / quantifier / resource / ...)
    where: str  # the call site in the game that keeps this state instead
...
def print_refusal_log() -> None:
    ...
    for i, r in enumerate(REFUSALS, 1):
        ...
        print(f"    shadow state now lives at: {r.where}")
