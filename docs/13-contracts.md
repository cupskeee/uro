...
Validation gauntlet, in order: emitter whitelist → payload schema → tier ceiling (ActorCreated ≤ T1, PlaceCreated = Site only) → **entity resolution** (`*Created` proposals matched against existing entities by name and alias — the embedding leg is designed but not built yet, see below; a plausible match links instead of creating — the anti-duplicate contract, with `EntityMerged` (`12`) as the after-the-fact correction) → **provenance & consequence gating** (trust model above) → contradiction check against `truth=true` claims and hard state. Everything past this point is downgrade-or-drop, never replan.
