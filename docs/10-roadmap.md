...
- **Proof of Chronicler mode:** the outcome-bundle endpoint, bundle schema v1 (participants, witnesses, casualties, notable feats, loot, duration; `extra='forbid'` + version-pinned, D-41), rule-based distillation into the standard gauntlet — and a ~50-line toy auto-battler script as the external "game."
...
Multiplayer `PartyArbiter` (OQ-7, DONE — D-31; arbiter shapes + durable session lifecycle extended it under the same port — D-38/D-39, #9/#10) · full Chronicler-mode ingestion contract beyond the toy proof (OQ-12, hardened — D-32; the trusted-embedder distillation tier landed — D-41, #12; the untrusted parked-encounter registry stays reserved) · ~~module/scripting system for packs~~ (DONE as the declarative **Reaction Layer**, D-33/D-40/D-42/docs/17 — a pack ships `rules.yaml`/`agendas.yaml` data, no sandbox; extended with multi-ref scopes #11 and `$trigger`-aware quantified triggers #25) · ~~computation/scripting for packs~~ (the engine-owned **computation layer** — event-sourced integer counters that fork by construction — shipped when that use-case arrived: **DONE, Phase 10 / D-34 / docs/19**; only a WASM/unbounded-computation tier stays reserved behind `ports/module.py` at a sharper gate) · graph/vector store swap-ins if scale demands · more rulesets (`srd51` — a d20 sibling; the alien-ruleset generality probe itself is DONE, Phase 6/D-30) · NATS-backed distribution · subscription-OAuth auth strategies if ever reconsidered (removed at D-16) · anything platform-shaped (which is someone else's repo).
