...
2. **The ripple** — at fork/continue time and after such major in-play events, the History service runs an **adaptation pass** (`AdaptationApplied`, `caused_by=history, pass=adaptation`): query claims/threads invalidated or newly implied ("Vel's market festival thread → dead"; "refugee crisis thread → spawned"), commit the adjustments, scoped to ~2 edge-hops (OQ-8). **(PoC status: the deterministic `time_skip` commits the `AdaptationApplied` header only — no thread invalidation/spawn ripple, and there is no `threads` projection yet; the LLM-driven ripple is deferred past Phase 2.)**
