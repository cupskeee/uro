...
| B2 | **The computation / scripting tier (D-33 Stage B)** | **all 4** — Sable Court's **12-rule refusal log** (exact wished syntax) is the headline; Ironwake's 7, Seventh's alarm-meter, Hollowloop's RL-1 corroborate | ✅ **DONE (INC-C1+C2, D-34)** — engine-owned integer counters (`CounterChanged`→`proj_counters`, forking by construction — the shadow-state fix) + threshold/cross-entity-compare/count-edges + `world` scope; phase-end review passed (a counter-RMW concurrency lost-update fixed). WASM reserved at a sharper gate. **C3/C4/C5 now also DONE (#13):** `for_each` (one bounded loop + `$trigger` binding), `roll_table` (seeded weighted pick), `expire_claims` (rumor decay, migration 019); `RULES_API_VERSION` 3→4; phase-end review (24 agents) fixed 2 HIGH (recursion iteration DoS; `created_day` missing from `_SNAPSHOT_TABLES`). Still staged: C6 cascade (touches `react()`'s single-hop invariant) + computed-delta arithmetic (OQ-1). | **The evidence gate we set for Stage B fired.** New argument: refused counters live in game code, so `fork_branch` no longer covers game state — **the two flagship features undermine each other** (Sable G-10, Ironwake row 6) — which is why the fix is *event-sourced* counters, not shadow state. |
...
| B11 | **`append_and_react` gap's cousins** — `create_thread`/`add_edge` scope is single-dimension | Sable G-3/G-12, Ironwake, Seventh G-6 | ✅ **DONE (D-40)** — `Scope` gains multi-ref forms (`factions: [a, b]`), the gauntlet unions their members (least-privilege between one-faction and `world`, which shipped in D-34); a validator enforces exactly one jurisdiction. `RULES_API_VERSION` 2→3. The gauntlet returns `GauntletResult(events, drops)`: every refused action (out-of-scope / nonexistent / partial filter / over the `_MAX_ACTIONS` cap) records a `DroppedAction(rule_id, do, ref, reason)` — the dropped-action audit (logged per pass; returned for tests). The action fence is untouched (jurisdiction widened only). |
