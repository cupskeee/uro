...
*Being settled (Phase 6, D-30):* the alien second ruleset is built — `uro_pbta`, a PbtA-style 2d6 system (stats-as-modifier, 7/10 tiers, a harm clock, moves, no hp/ac). Building it in lockstep forced 64 audited d20 assumptions out of the "generic" port into `uro_basic` (leak report in D-30); the port now hosts both through one contract + runner. The inc 6.1 generalization has landed; this OQ moves fully out to `decisions.md` when the phase closes (registry/pack-binding + the acceptance test, inc 6.3/6.4). A `srd51` (d20-in-a-hat) was rejected as the probe — it proves nothing; a narrate-only null ruleset likewise (no encounter path).
