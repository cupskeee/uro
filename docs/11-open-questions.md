...
*Partially settled (Reaction Layer, D-33, 2026-07-10):* the MECHANISM is now built — the thread lifecycle (`dormant→offered→active→resolved/dead` via `ThreadStateChanged`) and off-screen faction/actor agendas are engine-driven by pack-authored declarative rules (`Engine.react` post-commit + `Engine.agenda_tick` at time-skip, `caused_by=module`). What STAYS OPEN: History's *own* adaptation ripple (the 2-edge-hop blast radius from a cataclysm — still author/test-driven, not an engine sweep), and deep off-screen SIMULATION (accumulating faction state — armies/resources/territory — which the declarative grammar deliberately cannot express; that's the reserved sandbox tier's remit, not the Reaction Layer's).
...
What does an external game declare and report, exactly? Domain-authority scope (what it may commit directly), outcome-bundle schema beyond v1, time mapping between the game's clock and `world_time`, distillation rules (game-declared feat thresholds vs. summarizer-role judgment), witness reporting obligations, and anti-abuse limits (a buggy or malicious game spamming legendary feats).
*Trust model HARDENED (Phase 8 → D-32), but the OQ stays OPEN:* `distill_outcome` is now trust-scoped — a bundle can't kill/loot/first-hand-witness a PC, a T2+ named actor, or a bystander outside its declared cast (protected/out-of-cast casualties downgrade to `truth=unknown` testimony); loot needs real ownership; feat.actor is entity-resolved; bundles are size-capped and replay-idempotent. An adversarial audit's 25 exploits (incl. an authed POST killing/looting arbitrary actors) are closed. STILL OPEN (the "full contract", deferred to a real external game): a persisted **parked-encounter registry** (Uro pre-declaring the authorized roster/nonce → non-self-attested scope), **outcome-bundle schema beyond v1** (v1 is `extra='forbid'`+pinned, D-41), **time mapping** (game clock ↔ `world_time`), game-declared feat thresholds, witness reporting obligations, and finer endpoint→campaign authority.

*(OQ-13 · "the second ruleset should be alien" — **SETTLED → D-30**, Phase 6. The alien `uro_pbta` (2d6, harm clock, moves, no hp/ac) is built; it forced 64 audited d20 assumptions out of the generic port. Moved to `decisions.md` per process.)*
