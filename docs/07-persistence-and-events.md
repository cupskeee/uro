...
Projections (`actors`, `places`, `factions`, `claims`, `threads`, `edges`, `beliefs`, `chronicle`, `memory_index`, `entity_index`) are rebuildable caches keyed by branch (current-state tables are per-branch; the `edges` table additionally carries per-commit validity for point-in-time reads), versioned by projector code hash so schema evolution = replay, not migration surgery on truth. The `edges` table is the whole graph story for MVP: `(edge_id, branch_id, src, rel_type, dst, weight, valid_from_commit, valid_to_commit)` — temporal validity gives "relations at commit X" for materialization. The `ProjectionStore` read port is `state_at(branch, commit)`-shaped, so core never bakes in branch-locality even though the MVP adapter answers current-state reads from per-branch rows (a future copy-on-write adapter changes behind that port, not in the engines).

**Entity-resolution index.** `entity_index(branch_id, entity_id, content_hash)` points at embedded name/aliases/one-line-descriptor vectors (in the same `embeddings(content_hash, vector)` table as memory, but a *separate corpus*). A side-effecting projector writes it on `ActorCreated`/`PlaceCreated`/`FactionCreated`/`EntityAliasAdded`; entity resolution (`13`) kNN-matches a candidate name against it before accepting any `*Created`. This is what lets resolution dedup even seeded entities never yet narrated (which have zero memory chunks). Copied on fork like `memory_index`; never re-embedded.
